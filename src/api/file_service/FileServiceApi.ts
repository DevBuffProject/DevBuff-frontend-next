...
export default class FileServiceApi {
    ...
    async getUserAvatar (uuid : string ) : Promise<string> {
        return  await this.httpClient.get(`${FileServiceApi.BASE_PATH}/avatar/${uuid}`)
    }

    async getIdeaLogo (ideaId : string) : Promise<string> {
        return await this.httpClient.get(`${FileServiceApi.BASE_PATH}/idea/${ideaId}/logo`)
    }

    async getUserFileName (userId : string, fileName : string) : Promise<string> {
    ...
    async updateAvatar (image : string) : Promise<string> {
