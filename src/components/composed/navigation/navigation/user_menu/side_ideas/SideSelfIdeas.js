    ...
    return(
        <>
            <span className={'text-xxs opacity-50 font-montserratBold md:block flex justify-center items-center'}>Идеи</span>
                <ul className={'p-0 mt-1 flex  flex-col md:items-start items-center md:justify-start justify-center gap-8 border-b pb-5'}>
                    {
                        selfIdeas.map((idea,idx)=>{
                            return(
                                <li key={idx} className={'flex items-center justify-center md:gap-10 gap-4 group cursor-pointer'}>
                                    <div className={'w-8  h-7 p-0 m-0 opacity-60 rounded overflow-hidden'}>
                                        <Image priority src={`${service.getLogo(idea.id)}`} width={30} height={30} layout={'responsive'} unoptimized  />
                                    </div>
                                    <span className={'md:text-xs text-x2s opacity-60 hidden md:block font-montserratBold'}>{idea.name}</span>
                    ...
                    <li className={'flex items-center md:pb-2 md:pt-2 pb-0 pt-0 md:gap-2 gap-1 group cursor-pointer'}>
                        <div className={'w-5 p-0 m-0 opacity-60'}>
                            <Image src={'/icons/plus.svg'} width={20} height={20} />
                        </div>
                        <span className={'text-xs opacity-60 md:bg-transparent bg-blue-500 p-1  md:p-0 text-white md:text-black hidden md:block font-montserratBold'}>Рассказать идею</span>
                        <div className={'absolute right-5 opacity-0  group-hover:opacity-40 md:block hidden group-hover:visible transition ease-in-out duration-500'}>
